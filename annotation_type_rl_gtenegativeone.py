#!/usr/bin/env python3
"""
Annotation Type Reinforcement Learning Training Script for @GTENegativeOne
Uses binary RL models to train a separate model for predicting @GTENegativeOne annotation placement.
"""
...
class AnnotationTypeTrainer:
    ...
    def __init__(self, annotation_type='@GTENegativeOne', base_model_type='gcn', learning_rate=0.001, device='cpu'):
    ...
    def _extract_annotation_type_features(self, node, cfg_data):
        ...
        # Add annotation-specific features for @GTENegativeOne
        features.extend([
            float('capacity' in label.lower()),  # is_capacity_variable
            float('limit' in label.lower()),  # is_limit_variable
            float('bound' in label.lower()),  # is_bound_variable
            float('>' in label),  # has_greater_than
            float('>=' in label),  # has_greater_equal
            float('check' in label.lower()),  # is_check_related
        ])
    ...
    def _should_have_annotation_type(self, node):
        ...
        gte_neg_one_indicators = ['capacity', 'limit', 'bound', 'check', 'threshold', 'max']
...
def main():
    parser = argparse.ArgumentParser(description=f'Training for @GTENegativeOne annotation type')
