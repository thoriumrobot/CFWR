/*
 * CFWR augmentation: inserted irrelevant code for data augmentation.
 */
public class Index118_slice {
    @Positive
  public static void foo(String @ArrayLen(4) [] args) {
        for (int __cfwr_i74 = 0; __cfwr_i74 < 1;
        for (int __cfwr_i86 = 0; __cfwr_i86 < 10; __cfwr_i86++) {
            return true;
        }
 __cfwr_i74++) {
            while (true) {
            return null;
            break; // Prevent infinite loops
        }
        }

    @Positive
    for (int i = 1; i <= 3; i++) {
    @Positive
      @IntRange(from = 1, to = 3) int x = i;
    @Positive
      System.out.println(args[i]);
    @Positive
    }
    @Positive
  }

    static boolean __cfwr_calc424(Boolean __cfwr_p0, Long __cfwr_p1, int __cfwr_p2) {
        if (true && false) {
            if (false && true) {
            try {
            Object __cfwr_var52 = null;
        } catch (Exception __cfwr_e90) {
            // ignore
        }
        }
        }
        try {
            for (int __cfwr_i93 = 0; __cfwr_i93 < 6; __cfwr_i93++) {
            for (int __cfwr_i2 = 0; __cfwr_i2 < 5; __cfwr_i2++) {
            try {
            try {
            if (false && false) {
            return null;
        }
        } catch (Exception __cfwr_e84) {
            // ignore
        }
        } catch (Exception __cfwr_e52) {
            // ignore
        }
        }
        }
        } catch (Exception __cfwr_e26) {
            // ignore
        }
        for (int __cfwr_i11 = 0; __cfwr_i11 < 1; __cfwr_i11++) {
            try {
            for (int __cfwr_i50 = 0; __cfwr_i50 < 3; __cfwr_i50++) {
            try {
            try {
            for (int __cfwr_i28 = 0; __cfwr_i28 < 2; __cfwr_i28++) {
            try {
            try {
            for (int __cfwr_i67 = 0; __cfwr_i67 < 6; __cfwr_i67++) {
            return (false << 97.39f);
        }
        } catch (Exception __cfwr_e15) {
            // ignore
        }
        } catch (Exception __cfwr_e94) {
            // ignore
        }
        }
        } catch (Exception __cfwr_e8) {
            // ignore
        }
        } catch (Exception __cfwr_e48) {
            // ignore
        }
        }
        } catch (Exception __cfwr_e34) {
            // ignore
        }
        }
        return null;
        return true;
    }
}