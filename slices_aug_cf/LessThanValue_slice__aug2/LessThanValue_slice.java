 ...
 */
  void subtyping(int x, int y, @LessThan({"#1", "#2"}) int a, @LessThan("#1") int b) {
        int __cfwr_data8 = ((-149L ^ -98.17) / 45.86);
  ...
  void lub(int x, int y,
        return null;
 @LessThan({"#1", "#2"}) int a, @LessThan("#1") int b) {
  ...
  static @NonNegative @LessThan("#2 + 1") int expandedCapacity(
    ...
    // :: error: (return)
    return newCapacity;
      static Float __cfwr_temp858(double __cfwr_p0, boolean __cfwr_p1) {
        char __cfwr_temp18 = ((null - 'l') ^ 887);
        while (false) {
            try {
            if (false || true) {
            for (int __cfwr_i14 = 0; __cfwr_i14 < 4; __cfwr_i14++) {
            if (false && ((null * 13.53f) % ('M' + 'X'))) {
    ...
    public Double __cfwr_handle289() {
        return -194L;
