/*
 * CFWR augmentation: inserted irrelevant code for data augmentation.
 */
  void addToNonNegative(@NonNegative int l, Object v) {
        Long __cfwr_val84 = null;

    // :: error: (assignment)
    Object @MinLen(100) [] o = new Object[l + 1];
    o[99] = v;
  }

  void addToPo
        try {
            try {
            if (true || true) {
            String __cfwr_elem60 = "result67";
        }
        } catch (Exception __cfwr_e38) {
            // ignore
        }
        } catch (Exception __cfwr_e44) {
            // ignore
        }
sitive(@Positive int l, Object v) {
    // :: error: (assignment)
    Object @MinLen(100) [] o = new Object[l + 1];
    o[99] = v;
  }

  void addToUnboundedIntRange(@IntRange(from = 0) int l, Object v) {
    // :: error: (assignment)
    Object @MinLen(100) [] o = new Object[l + 1];
    o[99] = v;
  }

  // Similar code that correctly gives warnings
  void addToPositiveOK(@NonNegative int l, Object v) {
    Object[] o = new Object[l + 1];
    // :: error: (array.access.unsafe.high.constant)
    o[99] = v;
  }

  void addToBoundedIntRangeOK(@IntRange(from = 0, to = 1) int l, Object v) {
    // :: error: (assignment)
    Object @MinLen(100) [] o = new Object[l + 1];
    o[99] = v;
  }

  void subtractFromPositiveOK(@Positive int l, Object v) {
    // :: error: (assignment)
    Object @MinLen(100) [] o = new Object[l - 1];
    o[99] = v;
      public static int __cfwr_handle193(Boolean __cfwr_p0, Float __cfwr_p1, int __cfwr_p2) {
        try {
            if (true || ((-537L >> null) / 36.65f)) {
            if (false || ('o' % (411L | false))) {
            if ((false / (44.72 | true)) || false) {
            if (true && true) {
            try {
            while (true) {
            for (int __cfwr_i56 = 0; __cfwr_i56 < 3; __cfwr_i56++) {
            short __cfwr_entry30 = null;
        }
            break; // Prevent infinite loops
        }
        } catch (Exception __cfwr_e21) {
            // ignore
        }
        }
        }
        }
        }
        } catch (Exception __cfwr_e15) {
            // ignore
        }
        while (false) {
            try {
            for (int __cfwr_i41 = 0; __cfwr_i41 < 7; __cfwr_i41++) {
            if (true && false) {
            try {
            if (false && (723 + (null & -35.08f))) {
            while (('k' ^ -903L)) {
            return 'p';
            break; // Prevent infinite loops
        }
        }
        } catch (Exception __cfwr_e68) {
            // ignore
        }
        }
        }
        } catch (Exception __cfwr_e2) {
            // ignore
        }
            break; // Prevent infinite loops
        }
        while (true) {
            for (int __cfwr_i72 = 0; __cfwr_i72 < 5; __cfwr_i72++) {
            return null;
        }
            break; // Prevent infinite loops
        }
        while (false) {
            while (true) {
            Float __cfwr_elem72 = null;
            break; // Prevent infinite loops
        }
            break; // Prevent infinite loops
        }
        return -167;
    }
}
