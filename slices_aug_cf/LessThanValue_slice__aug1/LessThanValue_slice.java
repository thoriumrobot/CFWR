...
({"#1", "#2"}) int a, @LessThan("#1") int b) {
  ...
  static @NonNegative @LessThan("#2 + 1") int expandedCapacity(
    ...
    protected static Double __cfwr_handle536() {
        return "temp8";
        return null;
    }
    static double __cfwr_handle221() {
        return null;
    ...
    protected static boolean __cfwr_helper164() {
        if ((null * null) || true) {
            try {
            Long __cfwr_elem23 = null;
        } catch (Exception __cfwr_e8) {
            // ignore
        }
        }
        return null;
        Character __cfwr_entry61 = null;
        for (int __cfwr_i57 = 0; __cfwr_i57 < 2; __cfwr_i57++) {
            return null;
        }
        return ((651L | -892L) | (true % null));
    }
}