#!/usr/bin/env python3
"""
Annotation Type Reinforcement Learning Training Script for @Positive
Uses binary RL models to train a separate model for predicting @Positive annotation placement.
"""
...
class AnnotationTypeTrainer:
    ...
    def _extract_annotation_type_features(self, node, cfg_data):
        ...
        # Add annotation-specific features for @Positive
        features.extend([
            ...
            float('>' in label),  # has_greater_than
            float('=' in label and '==' not in label),  # has_assignment
            float('return' in label.lower()),  # is_return_statement
        ])
    ...
    def _should_have_annotation_type(self, node):
        ...
        if self.annotation_type == '@Positive':
            # @Positive: for values that must be > 0
            positive_indicators = ['count', 'size', 'length', 'capacity', 'limit']
            return any(indicator in label for indicator in positive_indicators)
        
        elif self.annotation_type == '@NonNegative':
            # @NonNegative: for values that must be >= 0
            nonnegative_indicators = ['index', 'offset', 'position', 'loop']
            return any(indicator in label for indicator in nonnegative_indicators)
        
        elif self.annotation_type == '@GTENegativeOne':
            # @GTENegativeOne: for values that must be >= -1
            gte_neg_one_indicators = ['capacity', 'limit', 'bound', 'check']
            return any(indicator in label for indicator in gte_neg_one_indicators)
    ...
    def train(self, project_root, warnings_file, cfwr_root, num_episodes=50):
        ...
        mock_cfg_data = {
            'nodes': [
                ...
                {'id': 1, 'label': 'int count = 5;', 'node_type': 'variable', 'line': 11},
                {'id': 2, 'label': 'int index = 0;', 'node_type': 'variable', 'line': 12},
                {'id': 3, 'label': 'int capacity = 10;', 'node_type': 'variable', 'line': 13}
            ],
            'control_edges': [
                {'source': 0, 'target': 1},
                {'source': 1, 'target': 2},
        ...
        self.save_training_stats(f'models_annotation_types/{self.annotation_type.replace("@", "").lower()}_stats.json')
...
def main():
    ...
    logger.info("@Positive annotation type training completed successfully")
